...
for anno_file_name in os.listdir(train_anno_dir):
    ...
    mminx = max(1, mminx - 25)
    mmaxx = min(int(width) - 1, mmaxx + 25)
    mminy = max(1, mminy - 25)
    mmaxy = min(int(height) - 1, mmaxy + 25)
    elem1 = dom.createElement('bndbox')
    ...
    fileHandle = open(plate_box_anno_dir+target_anno_name, 'w')
