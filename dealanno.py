...
for anno_file_name in os.listdir(anno_dir):
    ...
    filename = datetime.datetime.now().strftime('%m_%d_') + str(count) + '.jpg'#filenamelist[0].childNodes[0].data
    ...
    # 读写文件的句柄
    target_anno_name = datetime.datetime.now().strftime('%m_%d_') + str(count) + '.xml'
